...
DOWNLOAD_DIR=/shared/projects/mirage2/video_inpaint_octo_oxe
CONVERSION_DIR=/shared/projects/mirage2/merged_r2r_octo_oxe
N_WORKERS=20                  # number of workers used for parallel conversion --> adjust based on available RAM
MAX_EPISODES_IN_MEMORY=40    # number of episodes converted in parallel --> adjust based on available RAM
...
DATASET_TRANSFORMS=(
    "stanford_hydra_dataset_converted_externally_to_rlds 0.1.0 aug_merge"
)
...
for tuple in "${DATASET_TRANSFORMS[@]}"; do
  ...
  python3 ../modify_rlds_dataset.py --dataset=$DATASET --data_dir=$DOWNLOAD_DIR --target_dir=$CONVERSION_DIR --mods=$TRANSFORM --n_workers=$N_WORKERS --max_episodes_in_memory=$MAX_EPISODES_IN_MEMORY --device cuda:0 --source_robot Franka_2 --target_robot UR5 
done
