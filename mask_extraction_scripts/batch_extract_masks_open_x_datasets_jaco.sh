...
DATASET_TRANSFORMS=(
    "jaco_play 0.1.0 robot_mask_generator"
)
...
for tuple in "${DATASET_TRANSFORMS[@]}"; do
  ...
  python3 ../modify_rlds_dataset.py --dataset=$DATASET --data_dir=$DOWNLOAD_DIR --target_dir=$CONVERSION_DIR --mods=$TRANSFORM --n_workers=$N_WORKERS --max_episodes_in_memory=$MAX_EPISODES_IN_MEMORY --device cuda:5 --source_robot Jaco --target_robot Franka 
done
